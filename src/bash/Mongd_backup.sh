...
BACKUP_PATH="/var/www/db_backup"  #"/path/to/backup/directory" # do not include trailing slash
FILE_NAME="SHOUT_BACKUP" #defaults to [currentdate].tar.gz ex: 2011-12-19.tar.gz
...
if [ -d "$BACKUP_PATH" ]; then
    ...
    if [ "$USERNAME" != "" -a "$PASSWORD" != "" ]; then
        $MONGO_DUMP_BIN_PATH --db="$DBASE" --host $HOST:$PORT -u $USERNAME -p $PASSWORD --out $TMP_BACKUP_DIR >> /dev/null
    else
        $MONGO_DUMP_BIN_PATH --db="$DBASE" --host $HOST:$PORT --out $TMP_BACKUP_DIR >> /dev/null
    fi
